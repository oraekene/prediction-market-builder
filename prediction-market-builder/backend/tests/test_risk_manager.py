...
@pytest.mark.asyncio
async def test_risk_approves_good_signal():
...
@pytest.mark.asyncio
async def test_risk_rejects_low_confidence():
...
@pytest.mark.asyncio
async def test_risk_rejects_max_drawdown():
...
@pytest.mark.asyncio
async def test_evaluate_rules_none_returns_default_approved():
    result = await RiskManager(RiskProfile(rules=[], min_confidence=0.3)).evaluate_trade(
...
@pytest.mark.asyncio
async def test_max_drawdown_condition_triggers():
    rules = [
        {"condition": {"type": "max_drawdown", "params": {"threshold": 0.1}}, "action": {"type": "reject"}}
    ]
    result = await RiskManager(RiskProfile(rules=rules)).evaluate_trade(
...
@pytest.mark.asyncio
async def test_max_drawdown_condition_does_not_trigger():
    rules = [
...
@pytest.mark.asyncio
async def test_min_confidence_condition_triggers():
    rules = [
...
@pytest.mark.asyncio
async def test_max_position_size_condition_triggers():
    rules = [
...
@pytest.mark.asyncio
async def test_always_condition_triggers_reject():
    rules = [
...
@pytest.mark.asyncio
async def test_approve_action_overrides_reject():
    rules = [
...
@pytest.mark.asyncio
async def test_scale_position_action():
    rules = [
...
@pytest.mark.asyncio
async def test_fixed_fraction_action():
    rules = [
...
@pytest.mark.asyncio
async def test_monitor_positions_stop_loss_triggers():
    ...
    result = await mgr.monitor_positions(portfolio, market)
    assert result["alert_count"] > 0
    assert any(a["type"] == "stop_loss" for a in result["alerts"])


@pytest.mark.asyncio
async def test_monitor_positions_take_profit_triggers():
    mgr = RiskManager(RiskProfile(stop_loss=0.1))
    portfolio = {
        "current_capital": 10000,
        "peak_capital": 10000,
...
@pytest.mark.asyncio
async def test_monitor_positions_no_alerts():
    mgr = RiskManager(RiskProfile(stop_loss=0.1))
    portfolio = {
        "current_capital": 10000,
        "peak_capital": 10000,
...
@pytest.mark.asyncio
async def test_monitor_positions_empty_positions():
    mgr = RiskManager(RiskProfile(stop_loss=0.1))
...
@pytest.mark.asyncio
async def test_no_rule_match_uses_fallback_profile():
    mgr = RiskManager(RiskProfile(min_confidence=0.6))
    result = await mgr.evaluate_trade(
        market={"current_odds": 0.55},
        signal={"probability": 0.7, "confidence": 0.3, "market_odds": 0.55},
