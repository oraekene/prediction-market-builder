...
class TestWebSocketCommands:
    ...
    def test_connect_and_disconnect(self):
        with TestClient(app) as client:
            with client.websocket_connect("/api/research/ws/research/test-id"):
                pass
    ...
    def test_pause(self):
        ...
        with TestClient(app) as client:
            ...
            with client.websocket_connect("/api/research/ws/research/test-id") as ws:
    ...
    @patch("app.routers.research.async_session")
    def test_resume_with_scheduler_no_db_record(self, mock_async_session):
        mock_scheduler = AsyncMock()
        mock_db = AsyncMock()
        ...
        with TestClient(app) as client:
            ...
            with client.websocket_connect("/api/research/ws/research/test-id") as ws:
                ...
                data = ws.receive_json()
                assert data["type"] == "resumed"
        mock_scheduler.start_session.assert_not_awaited()

    @patch("app.routers.research.async_session")
    def test_resume_with_scheduler_and_db_record(self, mock_async_session):
        mock_scheduler = AsyncMock()
        mock_db = AsyncMock()
        mock_async_session.return_value.__aenter__.return_value = mock_db
        mock_result = MagicMock()
