...
export default function SafeWalletDashboard() {
  ...
  const handleTransfer = async () => {
    if (!transferForm.wallet_id || !transferForm.amount || !transferForm.source) return
    await transferToSafe.mutateAsync({
      wallet_id: transferForm.wallet_id,
      amount: parseFloat(transferForm.amount),
      source: transferForm.source,
    })
    setTransferForm({ wallet_id: '', amount: '', source: '' })
  ...
  const handleCreateWallet = async () => {
    ...
    setCreateForm({ name: '', currency: 'USDC', address: '' })
          ...
          {walletsLoading ? (
            ...
            wallets.map((wallet) => (
              ...
              >
                ...
                <button
                  onClick={() => {
                    setTransferForm((f) => ({ ...f, wallet_id: wallet.id }))
