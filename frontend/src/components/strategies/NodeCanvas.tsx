...
const nodeTypes = {
  ...
  execution: ActionNode,
}

export default function NodeCanvas(props: NodeCanvasProps) {
  const { initialNodes, initialEdges, onSave, onNodeSelect } = props
  ...
  // Initial nodes/edges are applied on mount only. Remount the canvas (via
  // React key) when switching strategies so user edits are never clobbered
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      setNodes((nds) => nds.concat(newNode as any) as any)
    },
    [setNodes],
  )
