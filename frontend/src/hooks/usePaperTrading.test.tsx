...
describe('usePlacePaperOrder', () => {
  ...
  it('calls placePaperOrder on mutation', async () => {
    const order = { wallet_id: 'w-1', market_id: 'm-1', side: 'buy', amount: 100, price: 0.55 }
...
describe('useSyncResolutions', () => {
  ...
  it('calls syncPaperResolutions on mutation', async () => {
    ...
    const resolutions = [
      { market_id: 'order-1', platform: 'polymarket', outcome: 'yes' },
      { market_id: 'order-2', platform: 'polymarket', outcome: 'no' },
    ]
    result.current.mutate(resolutions)
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(mockSyncPaperResolutions).toHaveBeenCalledWith(resolutions, expect.any(Object))
  })
})
